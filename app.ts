...
//Make one questan properties by use type alias
export type questan=
{
    questan:string,
    option:string[],
...
//questan collectivly call Questan
export type quiz=
{
    questan:questan[]
}
...
export function shuffle<s>(questanShuffle:s[])
{
...
export function runQuiz(quiz:quiz,quantity:number)
{
        ...
        ()=>
            {
                ...
                function askquestan(index:number)
                {
...
Questan ${index+1} : ${questan.questan}`);
                        ...
                        readlines.question(`            Enter your answer (1,2,3,etc.): `,
                            (Answer)=>
                                {
